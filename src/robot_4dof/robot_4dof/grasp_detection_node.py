#!/usr/bin/python3
"""
Grasp Detection Node for 4 DoF Robot — wraps AnyGrasp SDK with a fallback heuristic mode.
...
class GraspDetectionNode(Node):
    """Grasp detection node with AnyGrasp SDK or fallback heuristic (4 DoF)."""
    ...
    def __init__(self):
        ...
        self.get_logger().info(f"Grasp Detection Node (4 DoF) started — mode: {mode}")
    ...
    def _detect_callback(self):
        ...
        print("─" * 96)
        print(f" 🤖 Detected {len(grasps)} Grasps on 3D Objects (4 DoF with Wrist Yaw):")
